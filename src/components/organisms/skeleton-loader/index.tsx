...
export const SkeletonLoader = ({ children, backgroundColor, highlightColor }) => {
  ...
  const animatedStyles = useAnimatedStyle(() => ({
    transform: [
      {
        translateX: interpolate(shared.value, [0, 1], [layout ? -layout.width : 0, layout ? layout.width : 0]),
      },
    ],
  }));
  ...
  return (
    <MaskedView style={{ height: layout.height, width: layout.width }} maskElement={<View>{children}</View>}>
      <View style={[styles.background, { backgroundColor: backgroundColor || BACKGROUND_COLOR }]} />
      <Reanimated.View style={[animatedStyles, StyleSheet.absoluteFill]}>
        ...
        >
          <View style={[StyleSheet.absoluteFill, { backgroundColor: highlightColor || HIGHLIGHT_COLOR }]} />
        </MaskedView>
      </Reanimated.View>
    </MaskedView>
  );
